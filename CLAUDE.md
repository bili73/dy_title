...
1. **真机**：USB 调试已开，`adb devices` 能看到；livelite **需先手动登录一次**。udid 默认 `"auto"`（`config.ADB_CONFIG.udid`）自动取第一个在线设备，多设备时告警需写死。
...
| `TemplateMatcher` | OpenCV `matchTemplate`（Canny 边缘匹配）定位**无文字图标**（如参数入口的表盘图标 `param_icon*.png`）。**多尺度**：按屏宽主缩放 + ±10% 兜底，模板不绑死分辨率；抗颜色/亮度差异。 |
...
- **dp 换算层**：所有「相对锚点的像素偏移」和「配对容差」用 **dp** 表达，`AdbController.dp(n)` 按设备实际 dpi 换算 px（`px = round(n*dpi/160)`）。dp 值集中在 `config.DETAIL_CONFIG`（坐标字段）和 `config.LIST_OFFSETS_DP`（列表页偏移）；`DouyinCrawler._cache_dp_thresholds()` 启动时一次性换算缓存到 `self._px`，使用处读缓存（零运行时 adb）。基准 480dpi 下 1dp=3px。
- **多尺度模板匹配**：`TemplateMatcher.find` 按屏宽算主缩放因子 + ±10% 多尺度，`param_icon*.png` 模板不再绑死 1080 宽。
- **udid 自动识别**：`config.ADB_CONFIG.udid="auto"` → `AdbController._resolve_udid()` 取 `adb devices` 第一个在线设备。
- **改坐标阈值**：调 `config.DETAIL_CONFIG` / `LIST_OFFSETS_DP` 的 dp 值（注释标了 `[dp]` 和基准像素对照），别在方法体里写死 px。与分辨率无关的（秒数/次数/字数/置信度）保持原单位。
...
2. 拍同款入口/选图/结果页布局变化时，按文件分工调：
   - `config.py`：adb 路径、`scan_activity`、坐标阈值（单位 dp，见「设备分辨率/DPI 适配」）、等待秒数。`udid` 默认 `auto` 一般不用动。
